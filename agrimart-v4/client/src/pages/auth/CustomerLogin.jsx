...
const CustomerLogin = () => {
  ...
  const handleLogin = async (e) => {
    ...
    try {
      ...
      else if (data.role === 'dealer' || data.role === 'farmer') navigate('/supplier-dashboard/home');
      else navigate('/');
    } catch (err) {
      ...
      <div style={s.card}>
        <div style={s.header}>
          ...
          <div>
            ...
            <div style={s.logoSub}>Sign in to your account</div>
          </div>
        </div>
        ...
        <div style={s.footer}>
          <span>New customer? <Link to="/signup" style={s.link}>Register Free</Link></span>
          <span>Seller / Farmer? <Link to="/supplier-login" style={s.link}>Login here</Link></span>
        </div>
      </div>
    </div>
  );
...
const s = {
  ...
  input: { width: '100%', padding: '12px 14px', border: '1.5px solid #c8e6c9', borderRadius: '8px', fontSize: '14px', outline: 'none', fontFamily: 'Outfit, sans-serif', boxSizing: 'border-box', transition: 'border 0.2s' },
